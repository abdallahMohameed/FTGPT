...
public class StatefulChatService : IDisposable
{
    ...
    //    await foreach (var output in _session.ChatAsync(userInput, new Common.InferenceParams()
    //    {
    //        RepeatPenalty = 1.0f,
    //        AntiPrompts = new string[] { "User:" },
    //    }))
    ...
    //    return result;
    //}
    public async IAsyncEnumerable<string> StreamResults(SendMessageInput input)
    {
        var userInput = input.Text;
        if (!_continue)
        {
